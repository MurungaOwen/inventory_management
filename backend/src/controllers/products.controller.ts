...
export class ProductsController {
  ...
  public getProductById = asyncHandler(async (req: Request, res: Response) => {
    const productId = req.params.id as string; // Ensure productId is a string
  ...
  public createProduct = asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .status(201)
      .json({ data: createProductData.toPersistence(), message: "created" });
  });

  public updateProduct = asyncHandler(async (req: Request, res: Response) => {
    const productId = req.params.id as string; // Ensure productId is a string
    const productData = req.body;
    const updateProductData = await this.productsService.updateProduct(
      productId, // Ensure productId is a string
      productData,
    );
    res
      .status(200)
      .json({ data: updateProductData.toPersistence(), message: "updated" });
  });
